...
export function getDashboardHtml(version: string): string {
  const footer =
    '\n<footer class="py-4 px-6 border-t border-edge text-center text-xs text-muted font-mono leading-relaxed">' +
    '<div>Whetstone MCP v' + version + ' · MIT License</div>' +
    '<div><a class="text-accent no-underline hover:underline" href="https://github.com/frontier-collective/whetstone-mcp">GitHub</a> · ' +
    '<a class="text-accent no-underline hover:underline" href="https://www.npmjs.com/package/@frontier-collective/whetstone-mcp">npm</a> · ' +
    'Built by <a class="text-accent no-underline hover:underline" href="https://github.com/frontier-collective">Frontier Collective</a></div>' +
    '</footer>\n';
...
<div id="modal-overlay" class="fixed inset-0 bg-black/70 z-[1000] flex items-center justify-center p-6" style="display:none" onclick="if(event.target===this)closeModal()">
  <div class="bg-card border border-edge rounded-xl w-full max-w-[720px] max-h-[calc(100vh-48px)] overflow-y-auto shadow-2xl" id="modal-content"></div>
</div>
</template>
`;
...
async function openRejection(id) {
  ...
  try {
    ...
    var html = '<div class="flex items-start justify-between p-5 pb-4 border-b border-edge sticky top-0 bg-card rounded-t-xl z-10">';
    html += '<div><div class="text-[11px] font-mono text-muted uppercase tracking-wide mb-1">Rejection</div>';
    html += '<h2 class="text-base font-semibold text-primary leading-snug">' + esc(r.description) + '</h2></div>';
    html += '<button class="bg-transparent border-none text-muted text-2xl cursor-pointer pl-4 leading-none shrink-0 hover:text-primary" onclick="closeModal()">\\u00D7</button></div>';
    html += '<div class="p-5">';
...
async function openConstraint(id) {
  ...
  try {
    ...
    var html = '<div class="flex items-start justify-between p-5 pb-4 border-b border-edge sticky top-0 bg-card rounded-t-xl z-10">';
    html += '<div><div class="text-[11px] font-mono text-muted uppercase tracking-wide mb-1">Constraint</div>';
    ...
    if (linked.length > 0) {
      ...
      for (var i = 0; i < linked.length; i++) {
        var lr = linked[i];
        html += '<div class="py-2 px-3 bg-raised rounded-md mb-1.5 cursor-pointer border border-transparent hover:border-accent" onclick="openRejection(\\'' + esc(lr.id) + '\\')">';
        ...
        html += '<button onclick="event.stopPropagation();unlinkFromConstraint(\\'' + esc(lr.id) + '\\',\\'' + esc(c.id) + '\\')" class="text-red border border-red bg-transparent rounded text-[10px] px-1.5 py-0.5 cursor-pointer shrink-0">Unlink</button>';
        html += '</div></div>';
      }
      html += '</div>';
    } else {
    ...
    if (linked.length === 0) {
      ...
      html += '<button onclick="deleteConstraint(\\'' + esc(c.id) + '\\')" class="text-red border border-red bg-transparent rounded-md text-[13px] py-2 px-4 cursor-pointer">Delete Constraint</button>';
